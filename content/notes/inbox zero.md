...
According to [[andy matuschak|Andy Matuschak]], inbox zero only makes sense if inboxes are small and when the number of [[open loops]] arriving constantly exceeds those that are processed. If the number of arriving open loops are quite low, they can still be processed on time without inbox zero.
